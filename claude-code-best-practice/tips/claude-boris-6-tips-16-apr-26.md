# 6 Tips for Getting More Out of Opus 4.7 ÔÇö From Boris Cherny

A thread of tips shared by Boris Cherny ([@bcherny](https://x.com/bcherny)), creator of Claude Code, on April 16, 2026 ÔÇö after dogfooding Opus 4.7 for the last few weeks.
...
After dogfooding Opus 4.7 for a few weeks, Boris has been feeling "incredibly productive" and shared six ways to get more out of the new model ÔÇö from permission automation to effort tuning to verification patterns.

<a href="https://x.com/bcherny"><img src="assets/boris-26-4-16/0.png" alt="Boris Cherny intro tweet ÔÇö dogfooding Opus 4.7" width="50%" /></a>

---

## 1/ Auto Mode ÔÇö No More Permission Prompts
...
This means no more babysitting while the model runs. More than that, it means you can run more Claudes in parallel ÔÇö if safe, you can switch focus to the next Claude.

Auto mode is now available for Opus 4.7 for Max, Teams, and Enterprise users. **Shift+Tab** to cycle between `Ask permissions` ÔåÆ `Plan mode` ÔåÆ `Auto mode` in the CLI, or choose it from the dropdown in Desktop or VS Code.
...
- **Lower effort** ÔÇö faster responses and lower token usage
- **Higher effort** ÔÇö the most intelligence and capability

The slider presents five levels: `low` ┬À `medium` ┬À `high` ┬À `xhigh` ┬À `max` ÔÇö Speed on the left, Intelligence on the right.
...
Finally, make sure Claude has a way to verify its work. This has always been important ÔÇö now 4.7 is 2-3x what you get out of Claude, so it's more important than ever.

Verification looks different depending on the task:

- **Backend work** ÔÇö have Claude run your server/service to test end-to-end
- **Frontend work** ÔÇö use the [Claude Chromium extension](https://code.claude.com/docs/en/chrome) to give Claude a way to control your browser
- **Desktop apps** ÔÇö use Computer Use
...
For long-running work, verification matters even more ÔÇö when you come back to a task, you know the code works.
...
- [Boris Cherny (@bcherny) on X ÔÇö April 16, 2026](https://x.com/bcherny)
