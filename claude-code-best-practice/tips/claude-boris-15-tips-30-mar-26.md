# 15 Hidden & Under-Utilized Features in Claude Code ÔÇö From Boris Cherny
...
Did you know Claude Code has a mobile app? Boris writes a lot of his code from the iOS app ÔÇö it's a convenient way to make changes without opening a laptop.
...
## 3/ /loop and /schedule ÔÇö Two of the Most Powerful Features
...
- `/loop 5m /babysit` ÔÇö auto-address code review, auto-rebase, and shepherd PRs to production
- `/loop 30m /slack-feedback` ÔÇö automatically put up PRs for Slack feedback every 30 mins
- `/loop /post-merge-sweeper` ÔÇö put up PRs to address code review comments he missed
- `/loop 1h /pr-pruner` ÔÇö close out stale and no longer necessary PRs
- ...and lots more!
...
- Think of it like asking someone to build a website but they aren't allowed to use a browser ÔÇö the result probably won't look good
- Give Claude a browser and it will write code and iterate until it looks good
- Boris uses the Chrome extension every time he works on web code ÔÇö it tends to work more reliably than other similar MCPs
...
`/branch` creates a branched conversation ÔÇö you are now in the branch. To resume the original, use `claude -r <original-session-id>`.
...
> dachshund ÔÇö German for "badger dog" (dachs + badger, hund + dog).
Ôåæ/Ôåô to scroll ┬À Space, Enter, or Escape to dismiss
```
...
- [Boris Cherny (@bcherny) on X ÔÇö March 30, 2026](https://x.com/bcherny/status/2038454336355999749)
