...
| Tool Use Examples | Schema alone can't express usage patterns | 72% ÔåÆ 90% accuracy | API, Foundry (GA) |

All features are **generally available** as of February 18, 2026.

**Strategic layering** ÔÇö start with your biggest bottleneck:
- Context bloat from tool definitions ÔåÆ Tool Search Tool
- Large intermediate results ÔåÆ Programmatic Tool Calling
- Web search noise ÔåÆ Dynamic Filtering
- Parameter errors ÔåÆ Tool Use Examples

---

## Programmatic Tool Calling (PTC)

<img src="assets/programmatic-tool-calling-diagram.svg" alt="PTC Diagram ÔÇö Traditional vs Programmatic Tool Calling" width="100%" />

### The Paradigm Shift

**Before (Traditional Tool Calling):**
```
User prompt ÔåÆ Claude ÔåÆ Tool call 1 ÔåÆ Response 1 ÔåÆ Claude ÔåÆ Tool call 2 ÔåÆ Response 2 ÔåÆ Claude ÔåÆ Tool call 3 ÔåÆ Response 3 ÔåÆ Claude ÔåÆ Final answer
```
Each tool call requires a full model round trip. 3 tools = 3 inference passes.

**After (Programmatic Tool Calling):**
```
User prompt ÔåÆ Claude ÔåÆ writes Python script ÔåÆ Script calls Tool 1, Tool 2, Tool 3 internally ÔåÆ stdout ÔåÆ Claude ÔåÆ Final answer
```
...
4. You provide the tool result ÔÇö it goes to the **running code**, not Claude's context
...
**Batch processing** ÔÇö process N items in 1 inference pass:
```python
...
**Early termination** ÔÇö stop as soon as success criteria are met:
```python
...
**Data filtering** ÔÇö reduce what Claude sees:
```python
...
- Tool results from programmatic calls are **not added to Claude's context** ÔÇö only final `stdout`
- Intermediate processing happens in code, not model tokens
- 10 tools programmatically Ôëê 1/10th the tokens of 10 direct calls
...
Web search and fetch tools dump full HTML pages into Claude's context window. Most of that content is irrelevant ÔÇö navigation, ads, boilerplate. Claude then reasons over all of it, wasting tokens and reducing accuracy.
...
**Before:**
```
Query ÔåÆ Search results ÔåÆ Fetch full HTML ├ù N pages ÔåÆ All content enters context ÔåÆ Claude reasons over everything
```

**After:**
```
Query ÔåÆ Search results ÔåÆ Claude writes filtering code ÔåÆ Code extracts relevant content only ÔåÆ Filtered results enter context
```
...
~85% reduction in tool definition tokens (77K ÔåÆ 8.7K in Anthropic's benchmarks).
...
Add `input_examples` to tool definitions ÔÇö concrete usage patterns beyond the schema.
...
- Focus on resolving ambiguity ÔÇö target behavioral clarity over schema completeness
- Show parameter correlations (e.g., `priority: "critical"` tends to have `assignee`)

### Results

72% ÔåÆ 90% accuracy on complex parameter handling in Anthropic's benchmarks.
...
3. Implement the tool result loop (pause ÔåÆ provide result ÔåÆ resume)
