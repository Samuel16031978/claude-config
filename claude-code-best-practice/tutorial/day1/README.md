# Day 1 ÔÇö Your First Conversation with Claude Code

[Back to Day 0 (Setup)](../day0/README.md)

---

You've installed Claude Code. Now what? This guide walks you through three levels of using it ÔÇö each one gives you more control over **how** Claude does its work.
...
> ­ƒºá **Think of it like** texting a friend who knows a lot. You ask "what's the weather in Karachi?" and they'll give you *an* answer ÔÇö but you have no idea if they checked a weather app, looked out their window, or just guessed from memory.
...
Claude will answer ÔÇö but **how** it answers is unpredictable. It might:
...
Every time you ask "what's the weather?", Claude might fetch data differently ÔÇö or not fetch real data at all. There's no guarantee it uses the same source or method twice.

---

## Level 2: Agents (The Specialist)

An **agent** is Claude playing a specific role ÔÇö like assigning a job title.

> ­ƒºá **Think of it like** a restaurant kitchen. Without an agent, you walk into a random kitchen and shout "make me pasta!" ÔÇö whoever hears you might boil instant noodles or make a five-course Italian meal. With an agent, you hire a **Pasta Chef** whose job description says: *"Always use fresh ingredients, always cook al dente, always plate it the same way."* Now you know exactly what you're getting, every single time.
...
This repo has a `weather-agent` ÔÇö its entire job is fetching Dubai's temperature. Here's what makes it different from just prompting:
...
Agents give you **predictability**. Same question ÔåÆ same approach ÔåÆ same quality. That's the advantage ÔÇö not that agents are smarter, but that they're **consistent**.
...
> ­ƒºá **Think of it like** a new employee's training manual. When someone joins your team, they have a role (agent), but they also go through specific training modules ÔÇö how to use the CRM, how to write a proposal, how to run a standup. Each training module is a **skill**. The role tells them *what* they are; the skills tell them *how* to do specific things.

Now think about a real person:

> **Shayan** has many skills:
> - Engineering skill ÔÇö can write code
> - Gaming skill ÔÇö knows game mechanics
> - Reading skill ÔÇö can digest and summarize long documents
>
...
```
Level 1: PROMPTING
You ÔåÆ "What's the weather?" ÔåÆ Claude figures it out somehow
                                (unpredictable method)

Level 2: AGENTS
You ÔåÆ Weather Agent ÔåÆ Always uses the same approach
                      (predictable method)

Level 3: SKILLS
You ÔåÆ Weather Agent ÔåÆ Uses weather-fetcher skill
...
For now, spend time at **Level 1** ÔÇö just prompt. Get comfortable asking Claude questions in the terminal. The more you use it, the more you'll notice tasks that would benefit from an agent.
