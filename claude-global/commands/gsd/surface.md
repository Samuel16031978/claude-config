...
Sub-commands: list ┬À status ┬À profile ┬À disable ┬À enable ┬À reset
</objective>
...
Disabled:
  utility:  health  stats  settings  ÔÇª
...
1. Read current surface: `readSurface(runtimeConfigDir)` ÔåÆ if null, seed from `readActiveProfile(runtimeConfigDir)`.
...
4. `writeSurface` ÔåÆ resolve layout ÔåÆ `applySurface`:
   ```js
   const layout = resolveRuntimeArtifactLayout(runtime, runtimeConfigDir, scope);
   applySurface(runtimeConfigDir, layout, manifest, CLUSTERS);
   ```
...
3. `writeSurface` ÔåÆ resolve layout ÔåÆ `applySurface`:
   ```js
   const layout = resolveRuntimeArtifactLayout(runtime, runtimeConfigDir, scope);
   applySurface(runtimeConfigDir, layout, manifest, CLUSTERS);
   ```
...
'skills'` ÔÇö they are derived from `configDir`, not the root for it.

```bash
# Claude Code ÔÇö global install
...
- Unknown cluster name ÔåÆ list valid cluster names, exit without writing.
- Unknown profile name ÔåÆ list known profiles (`core`, `standard`, `full`), exit.
- Missing `surface.cjs` ÔåÆ prompt: "Run `npm i -g get-shit-done` to reinstall GSD."
