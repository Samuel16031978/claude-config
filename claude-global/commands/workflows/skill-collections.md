...
# Workflow ÔÇö Skill Collections
...
- **Name**: `[Short Name](github-url)` ÔÇö use repo's short name (e.g., `mattpocock/skills`, or just `skills` if owner is the project), not the full owner/repo unless ambiguous
- **Ôÿà**: Star count rounded to `k` (e.g., 125k, 35k, 1.2k). Under 1000 show exact number
...
1. `README.md` ÔÇö the `## ­ƒº░ SKILL COLLECTIONS` table (note current stars and skill counts)
2. `changelog/skill-collections/changelog.md` ÔÇö previous changelog entries (may not exist yet)

---

## Phase 1: Launch Research Agent

**Immediately** spawn one `development-workflows-research-agent` covering all 5 repos. (The existing research agent is generic ÔÇö it counts skills/stars/etc. for any repo.)
...
>
> **Repo 1: anthropics/skills** (https://github.com/anthropics/skills) ÔÇö official Anthropic skills repo
> **Repo 2: wshobson/agents** (https://github.com/wshobson/agents) ÔÇö plugin-scoped skills (skills nested under domain plugins)
> **Repo 3: mattpocock/skills** (https://github.com/mattpocock/skills) ÔÇö TypeScript-focused
> **Repo 4: K-Dense-AI/scientific-agent-skills** (https://github.com/K-Dense-AI/scientific-agent-skills) ÔÇö science/research vertical
> **Repo 5: VoltAgent/awesome-agent-skills** (https://github.com/VoltAgent/awesome-agent-skills) ÔÇö curated awesome-list (links to external skills, not SKILL.md files in repo)
>
> For EACH repo, return:
>
> 1. **Stars** ÔÇö use GitHub API `https://api.github.com/repos/{owner}/{repo}`, read `stargazers_count`. Round to `k`.
> 2. **Skill count** ÔÇö count `SKILL.md` files in the repo via the GitHub git tree API:
>    `https://api.github.com/repos/{owner}/{repo}/git/trees/HEAD?recursive=1` and grep paths for `SKILL.md`.
>    - For `wshobson/agents`: skills are nested inside `plugins/<domain>/skills/` ÔÇö count all SKILL.md across all plugins.
...
> 3. **Notable changes** ÔÇö any significant additions or removals in last 30 days?
>
...
> NOTES: <anything unusual ÔÇö flat .md vs SKILL.md, deprecated skills, language variants, curated-list disclaimer>
...
When executing, edit the `## ­ƒº░ SKILL COLLECTIONS` table in `README.md`:
...
1. **One research agent, 5 repos** ÔÇö single message, parallel sub-fetches inside
2. **Never guess** ÔÇö use data from the agent only
...
6. **Round stars consistently** ÔÇö `k` suffix (125k, 35k, 1.2k). Under 1000 show exact
7. **Awesome-lists are different** ÔÇö for repos that link to external skills (VoltAgent), the count is "items listed in README", not files in repo; always annotate `(curated list)`
8. **Compare with previous changelog** ÔÇö mark items NEW, RECURRING, or RESOLVED
9. **Reuse `development-workflows-research-agent`** ÔÇö do NOT create a new agent
