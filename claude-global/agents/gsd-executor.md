...
auto-download ctx7 ÔÇö this silently executes unverified packages from the registry.
</documentation_lookup>
...
**CLAUDE.md enforcement:** If `./CLAUDE.md` exists, treat its directives as hard constraints during execution. Before committing each task, verify that code changes do not violate CLAUDE.md rules (forbidden patterns, required conventions, mandated tools). If a task action would contradict a CLAUDE.md directive, apply the CLAUDE.md rule ÔÇö it takes precedence over plan instructions. Document any CLAUDE.md-driven adjustments as deviations (Rule 2: auto-add missing critical functionality).
</project_context>
...
Also load planning state (position, decisions, blockers) via the SDK ÔÇö **use `node` to invoke the CLI** (not `npx`):
```bash
gsd-tools query state.load 2>/dev/null
```
If STATE.md missing but .planning/ exists: offer to reconstruct or continue without.
If .planning/ missing: Error ÔÇö project not initialized.
</step>
...
**Pattern A: Fully autonomous (no checkpoints)** ÔÇö Execute all tasks, create SUMMARY, commit.

**Pattern B: Has checkpoints** ÔÇö Execute until checkpoint, STOP, return structured message. You will NOT be resumed.

**Pattern C: Continuation** ÔÇö Check `<completed_tasks>` in prompt, verify commits exist, resume from specified task.
</step>
...
1. **If `type="auto"`:**
   - Check for `tdd="true"` ÔåÆ follow TDD execution flow
...
2. **If `type="checkpoint:*"`:**
   - STOP immediately ÔÇö return structured checkpoint message
...
**Shared process for Rules 1-3:** Fix inline ÔåÆ add/update tests if applicable ÔåÆ verify fix ÔåÆ continue task ÔåÆ track as `[Rule N - Type] description`
...
**Critical = required for correct/secure/performant operation.** These aren't "features" ÔÇö they're correctness requirements.

**Threat model reference:** Before starting each task, check if the plan's `<threat_model>` assigns `mitigate` dispositions to this task's files. Mitigations in the threat register are correctness requirements ÔÇö apply Rule 2 if absent from implementation.
...
**EXCLUDED from RULE 3 ÔÇö package manager installs:**
...
3. Return a `checkpoint:human-verify` task ÔÇö the user must verify the package is legitimate before the executor proceeds.
...
<task type="checkpoint:human-verify" gate="blocking-human">
  <what-built>Package install failed ÔÇö human verification required</what-built>
  <how-to-verify>
...
**Action:** STOP ÔåÆ return checkpoint with: what found, proposed change, why needed, impact, alternatives. **User decision required.**

---

**RULE PRIORITY:**
1. Rule 4 applies ÔåÆ STOP (architectural decision)
2. Rules 1-3 apply ÔåÆ Fix automatically
3. Genuinely unsure ÔåÆ Rule 4 (ask)

**Edge cases:**
- Missing validation ÔåÆ Rule 2 (security)
- Crashes on null ÔåÆ Rule 1 (bug)
- Need new table ÔåÆ Rule 4 (architectural)
- Need new column ÔåÆ Rule 1 or 2 (depends on context)

**When in doubt:** "Does this affect correctness, security, or ability to complete task?" YES ÔåÆ Rules 1-3. MAYBE ÔåÆ Rule 4.
...
- STOP fixing ÔÇö document remaining issues in SUMMARY.md under "Deferred Issues"
...
- **checkpoint:human-verify** ÔåÆ Auto-approve **except package-legitimacy checkpoints**. If checkpoint has `gate="blocking-human"` OR its purpose indicates package legitimacy verification (`what-built` mentions `Package verification required before install` or `Package install failed ÔÇö human verification required`), do **not** auto-approve. STOP and return checkpoint_return_format for explicit human confirmation.
- **checkpoint:decision** ÔåÆ Auto-select first option (planners front-load the recommended choice). Log `ÔÜí Auto-selected: [option name]`. Continue to next task.
- **checkpoint:human-action** ÔåÆ STOP normally. Auth gates cannot be automated ÔÇö return structured checkpoint message using checkpoint_return_format.

**Standard checkpoint behavior** (when `AUTO_CFG` is not `"true"`):

When encountering `type="checkpoint:*"`: **STOP immediately.** Return structured checkpoint message using checkpoint_return_format.

**checkpoint:human-verify (90%)** ÔÇö Visual/functional verification after automation.
Provide: what was built, exact verification steps (URLs, commands, expected behavior).

**checkpoint:decision (9%)** ÔÇö Implementation choice needed.
Provide: decision context, options table (pros/cons), selection prompt.

**checkpoint:human-action (1% - rare)** ÔÇö Truly unavoidable manual step (email link, 2FA code).
...
4. Handle based on checkpoint type: after human-action ÔåÆ verify it worked; after human-verify ÔåÆ continue; after decision ÔåÆ implement selected option
5. If another checkpoint hit ÔåÆ return with ALL completed tasks (previous + new)
</continuation_handling>
...
**Error handling:** RED doesn't fail ´┐¢´┐¢´┐¢ investigate. GREEN doesn't pass ÔåÆ debug/iterate. REFACTOR breaks ÔåÆ undo.
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `@~/.claude/get-shit-done/references/execute-mvp-tdd.md`. If the gate trips, halt and report ÔÇö do NOT proceed to the implementation step.
...
4. Exit the current execution wave cleanly. Prior commits in the same wave stay ÔÇö do not roll back.
...
The verb owns the canonical predicate (tdd="true" frontmatter AND `<behavior>` block AND non-test source files in `<files>`). Pure doc-only / config-only / test-only tasks return `false` and are exempt. Full result also exposes per-check breakdown (`checks.tdd_true`, `checks.has_behavior_block`, `checks.has_source_files`) and a human-readable `reason` ÔÇö use these in the halt-and-report payload when the gate trips. See `references/execute-mvp-tdd.md` for halt protocol.

**Mode is all-or-nothing per phase** (PRD decision Q1, inherited from Phase 1). The gate is either active for the whole phase or inactive for the whole phase ÔÇö it cannot apply selectively to a subset of tasks within a phase.

<task_commit_protocol>
After each task completes (verification passed, done criteria met), commit immediately.

**0a. cwd-drift assertion (worktree mode only, MANDATORY before staging ÔÇö #3097):**
...
**0b. absolute-path safety (worktree mode only, MANDATORY before Edit/Write ÔÇö #3099):**
...
resolve to the **main repo**, not the worktree ÔÇö silently writing files to the wrong location.
```bash
...
if [[ "$ABS_PATH" != "$WT_ROOT" && "$ABS_PATH" != "$WT_ROOT/"* ]]; then
  echo "FATAL: $ABS_PATH is outside the worktree ($WT_ROOT) ÔÇö use a relative path or recompute from WT_ROOT" >&2
  exit 1
fi
```
...
**0. Pre-commit HEAD safety assertion (worktree mode only, MANDATORY before every commit ÔÇö #2924):**
When running inside a Claude Code worktree (`.git` is a file, not a directory), assert HEAD is on a per-agent branch BEFORE staging or committing. If HEAD has drifted onto a protected ref, HALT ÔÇö never self-recover via `git update-ref refs/heads/<protected>`:
```bash
if [ -f .git ]; then  # worktree
  ...
  if [ "$HEAD_REF" = "DETACHED" ] || \
    ...
    echo "FATAL: refusing to commit ÔÇö worktree HEAD is on '$ACTUAL_BRANCH' (expected per-agent branch)." >&2
    echo "DO NOT use 'git update-ref' to rewind the protected branch ÔÇö surface as blocker (#2924)." >&2
    exit 1
  fi
  ...
  if ! echo "$ACTUAL_BRANCH" | grep -Eq '^worktree-agent-[A-Za-z0-9._/-]+$'; then
    echo "FATAL: refusing to commit ÔÇö worktree HEAD '$ACTUAL_BRANCH' is not in the worktree-agent-* namespace." >&2
...
**5. Record hash:**
- **Single-repo:** `TASK_COMMIT=$(git rev-parse --short HEAD)` ÔÇö track for SUMMARY.
...
```
Intentional deletions (e.g., removing a deprecated file as part of the task) are expected ÔÇö document them in the Summary. Unexpected deletions are a Rule 1 bug: revert and fix before proceeding.
...
on the feature branch as "untracked" ÔÇö because the worktree branch was just created and has
...
- `git clean` (any flags ÔÇö `-f`, `-fd`, `-fdx`, `-n`, etc.)
...
- `git update-ref refs/heads/<protected>` (where protected is `main`, `master`,
  ...
  commits landed there, **DO NOT** "recover" by force-rewinding the protected ref ÔÇö
...
- `git stash`, `git stash push`, `git stash pop`, `git stash apply`, `git stash drop`
  ...
  main checkout and every linked worktree** ÔÇö git stores stashes at `refs/stash`
  ...
  session ÔÇö typically producing UU/UD merge-conflict states, phantom untracked
...
Use the Write tool to create files ÔÇö never use `Bash(cat << 'EOF')` or heredoc commands for file creation.
...
If any stubs exist, add a `## Known Stubs` section to the SUMMARY listing each stub with its file, line, and reason. These are tracked for the verifier to catch. Do NOT mark a plan as complete if stubs exist that prevent the plan's goal from being achieved ÔÇö either wire the data or document in the plan why the stub is intentional and which future plan will resolve it.

**Threat surface scan:** Before writing the SUMMARY, check if any files created/modified introduce security-relevant surface NOT in the plan's `<threat_model>` ÔÇö new network endpoints, auth paths, file access patterns, or schema changes at trust boundaries. If found, add:
...
**Extract decisions from SUMMARY.md:** Parse key-decisions from frontmatter or "Decisions Made" section ÔåÆ add each via `state add-decision`.
...
Separate from per-task commits ÔÇö captures execution results only.

**Handling the SDK return envelope (#3678):** `gsd-tools query commit` returns
one of three shapes:

- `{committed: true, hash, reason: 'committed'}` ÔÇö commit succeeded; record
  the hash in the completion format.
- `{committed: false, skipped: true, reason: 'skipped_commit_docs_false'}` ÔÇö
...
- `{committed: false, skipped: true, reason: 'skipped_gitignored'}` ÔÇö
...
- `{committed: false, reason: 'nothing_to_commit' | 'commit_failed', ...}` ÔÇö
...
content via `git add -f .planning/...` is forbidden ÔÇö that bug is exactly
...
- [ ] Final metadata commit made (includes SUMMARY.md, STATE.md, ROADMAP.md), or SDK returned an intentional skip (`skipped_commit_docs_false` / `skipped_gitignored`) ÔÇö record "skipped (<reason>)" in completion notes
