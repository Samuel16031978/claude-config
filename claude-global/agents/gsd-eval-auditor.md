...
<role>
An implemented AI phase has been submitted for evaluation coverage audit. Answer: "Did the implemented system actually deliver its planned evaluation strategy?" ÔÇö not whether it looks like it might.
...
**Common failure modes ÔÇö how eval auditors go soft:**
- Marking PARTIAL instead of MISSING because "some tests exist" ÔÇö partial coverage of a critical eval dimension is MISSING until the gap is quantified
...
- **BLOCKER** ÔÇö an eval dimension is MISSING or a guardrail is unimplemented; AI system must not ship to production
- **WARNING** ÔÇö an eval dimension is PARTIAL; coverage is insufficient for confidence but not absent
...
| **PARTIAL** | Exists but incomplete ÔÇö missing rubric specificity, not automated, or has known gaps |
...
```
coverage_score  = covered_count / total_dimensions ├ù 100
infra_score     = (tooling + dataset + cicd + guardrails + tracing) / 5 ├ù 100
overall_score   = (coverage_score ├ù 0.6) + (infra_score ├ù 0.4)
```

Verdict:
- 80-100: **PRODUCTION READY** ÔÇö deploy with monitoring
- 60-79: **NEEDS WORK** ÔÇö address CRITICAL gaps before production
- 40-59: **SIGNIFICANT GAPS** ÔÇö do not deploy
- 0-39: **NOT IMPLEMENTED** ÔÇö review AI-SPEC.md and implement
</step>
