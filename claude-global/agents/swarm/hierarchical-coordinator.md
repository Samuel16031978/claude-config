...
## ­ƒÜ¿ MANDATORY MEMORY COORDINATION PROTOCOL

### Every spawned agent MUST follow this pattern:

```javascript
// 1´©ÅÔâú IMMEDIATELY write initial status
...
// 2´©ÅÔâú UPDATE progress after each delegation
mcp__claude-flow__memory_usage {
  action: "store",
...
// 3´©ÅÔâú SHARE command structure for workers
mcp__claude-flow__memory_usage {
  action: "store",
...
// 4´©ÅÔâú CHECK worker status before assigning
