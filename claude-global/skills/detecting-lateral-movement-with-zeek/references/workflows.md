# Detection Workflow ÔÇö Lateral Movement with Zeek
...
Network Traffic (Span/TAP)
    ...
    Zeek Sensor
         Ôöé
         Ôö£ÔöÇÔöÇ conn.log          (all connections)
         Ôö£ÔöÇÔöÇ smb_mapping.log   (SMB share access)
         Ôö£ÔöÇÔöÇ dce_rpc.log       (DCE/RPC calls)
         Ôö£ÔöÇÔöÇ ntlm.log          (NTLM authentication)
         Ôö£ÔöÇÔöÇ files.log          (file transfers)
         ÔööÔöÇÔöÇ notice.log        (Zeek-generated alerts)
```
...
Ôöé  Initial Triage  Ôöé
Ôöé                   Ôöé
Ôöé 1. Is source a    Ôöé
Ôöé    known admin    ÔöéÔöÇÔöÇYesÔöÇÔöÇÔûÂ Log & reduce priority
Ôöé    workstation?   Ôöé
Ôöé                   Ôöé
Ôöé 2. Is activity    Ôöé
Ôöé    during change  ÔöéÔöÇÔöÇYesÔöÇÔöÇÔûÂ Verify change ticket
Ôöé    window?        Ôöé
Ôöé                   Ôöé
Ôöé 3. Multiple       Ôöé
Ôöé    indicators?    ÔöéÔöÇÔöÇYesÔöÇÔöÇÔûÂ ESCALATE immediately
...
1. **Update baselines** ÔÇö Add legitimate admin share usage to allowlists
2. **Tune detections** ÔÇö Adjust thresholds based on false positive analysis
3. **Document findings** ÔÇö Update incident report with Zeek evidence
4. **Improve coverage** ÔÇö Deploy additional Zeek scripts for newly discovered TTPs
...
- Admin share access from non-admin workstation ÔåÆ Auto-isolate + ticket
- DCSync from non-DC ÔåÆ Emergency alert + auto-isolate
- NTLM Account Spray threshold exceeded ÔåÆ Auto-disable account + alert
