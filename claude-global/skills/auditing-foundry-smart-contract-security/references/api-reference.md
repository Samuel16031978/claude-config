...
Severity matrix (impact ├ù confidence):
...
## Aderyn (Cyfrin, Rust static analyzer ÔÇö complementary to Slither)
...
## Mythril (symbolic execution ÔÇö slow, use on critical contracts only)
...
### Invariant testing ÔÇö handler pattern (the important one)
