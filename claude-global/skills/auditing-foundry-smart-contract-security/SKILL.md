...
shipped to mainnet cannot be patched ÔÇö it can only be exploited. Most catastrophic
...
1. **Static analysis** ÔÇö `slither` (90+ detectors) and `aderyn` (Cyfrin, Rust) scan
   the AST/IR in seconds for known anti-patterns.
2. **Symbolic execution** ÔÇö `mythril` (optional, slow) explores execution paths and
   SMT-solves for deep arithmetic/reentrancy bugs.
3. **Property-based testing** ÔÇö `forge test` with **fuzzing** (`testFuzz_*`) and
...
4. **Manual review + key hygiene** ÔÇö a structured checklist (see
...
The skill is **dev-side and pre-deployment** ÔÇö it is run by the engineer building
...
- When wiring deployment scripts ÔÇö to verify keys are in an encrypted keystore, not `.env` plaintext.
...
- A Foundry project that **compiles** (`forge build` succeeds) ÔÇö analyzers need build artifacts.
...
```bash
# Slither ÔÇö full project (uses foundry.toml + remappings automatically)
slither . --json slither-report.json

# Aderyn ÔÇö Cyfrin Rust analyzer, complementary detectors
...
```bash
# Only on the highest-value contract(s) ÔÇö Mythril is path-explosive
...
### Step 4: Property-based testing ÔÇö fuzz + invariants
...
inputs, ghost variables, `targetContract(handler)`) ÔÇö not just unit tests. See
...
# Import the deploy key ONCE into an encrypted keystore ÔÇö never a plaintext PRIVATE_KEY env
...
drop confirmed false positives, rank by exploitability ├ù financial impact, and map
