...
**Submodule guard:** Verify you are not in a submodule:
```bash
...
If you have a tool like `EnterWorktree`, `WorktreeCreate`, a `/worktree` command, or a `--worktree` flag  use it and skip to Step 3. Only proceed to 1b if no native tool is available.
...
If `git worktree add` fails with sandbox denial: work in current directory instead.
