...
> **Shipped vs. aspirational.** The *concrete, in-CI* verification stack ÔÇö the 6 regression-guard jobs + the witness manifest + the tool-discoverability audit ÔÇö is real and runs on every push. The truth-scoring / auto-rollback / WebSocket-dashboard surface described later in this doc is partly shipped (`ruflo verify` runs the witness checks) and partly design ÔÇö treat the "CI Guards" section below as the authoritative current state.

## CI Guards ÔÇö what's actually shipped (current state)
...
|---|---|---|---|
| **1 ÔÇö install/behavioral smoke** | Exercise user-visible failure modes against a real build | `smoke-install-no-bsqlite` (npm install on platforms w/o prebuilds), `plugin-hooks-smoke` (#1859/#1862 ÔÇö hook flag parsing), `mcp-protocol-smoke` (#1874 ÔÇö HTTP MCP wire format), `memory-import-smoke` (#1883/#1884 ÔÇö WSL path + key sanitization), `mcp-roundtrip-smoke` (#1889 paired-tool round-trip + #1863 cli-no-crash + ADR-095 G2 consensus-transport) | ADR-102 |
| **1 ÔÇö discoverability gate** | Every MCP tool description must answer "use this over native when?" | `tool-descriptions-audit` ÔÇö `scripts/audit-tool-descriptions.mjs`, baseline at `verification/mcp-tool-baseline.json` (monotone-decreasing: noGuidance / tooShort / duplicates) | ADR-112 |
| **2 ÔÇö cryptographic witness** | Every documented fix's load-bearing marker must still be present in dist; Ed25519-signed, per-OS bundles | `witness-verify` (ubuntu/macos/windows) ÔÇö `plugins/ruflo-core/scripts/witness/verify.mjs` against `verification/<os>/manifest.md.json` | ADR-103 |
| **3 ÔÇö temporal history** | When was a regression introduced | `verification/<os>/history.jsonl` + `history.mjs` (`summary` / `regressions` / `timeline`) | ADR-103 |
...
# Witness manifest ÔÇö regenerate + verify
...
1. **Behavioral smoke** ÔåÆ write `plugins/ruflo-core/scripts/test-<name>.mjs`. Pattern: static dist-scan first (fast, always completes), behavioral probe second with an internal timeout + a process-level watchdog so CI never hangs. Add a step to the relevant job in `v3-ci.yml`.
2. **Static gate with a baseline** ÔåÆ write `scripts/audit-<name>.mjs` that scans, counts violations, and fails if the count exceeds a monotone-decreasing baseline in `verification/<name>-baseline.json`. Support `--update-baseline`. Add a CI job; wire it into `witness-verify` `needs[]` if it should gate `publish`.
3. **Documented-fix marker** ÔåÆ append `{ id, desc, file, marker }` to `verification/witness-fixes.json`, run `node scripts/regen-witness.mjs`. The marker must be a substring the fix specifically creates (not a generic pattern like `'function'`).
...
- `@noble/ed25519` (for the witness verifier ÔÇö a single runtime dep, `npm i @noble/ed25519`)
...
Overall Truth Score: 0.947 Ô£à
Trend: Ôåù´©Å +2.3% (7d)

Top Performers:
  verification-agent   0.982 Ô¡É
  code-analyzer       0.971 Ô¡É
  test-generator      0.958 Ô£à

Needs Attention:
  refactor-agent      0.821 ÔÜá´©Å
  docs-generator      0.794 ÔÜá´©Å

Recent Tasks:
  task-456  0.991 Ô£à  "Implement auth"
  task-455  0.967 Ô£à  "Add tests"
  task-454  0.743 ÔØî  "Refactor API"
```

#### Metrics Explained

**Truth Scores (0.0-1.0):**
- `1.0-0.95`: Excellent Ô¡É (production-ready)
- `0.94-0.85`: Good Ô£à (acceptable quality)
- `0.84-0.75`: Warning ÔÜá´©Å (needs attention)
- `<0.75`: Critical ÔØî (requires immediate action)

**Trend Indicators:**
- Ôåù´©Å Improving (positive trend)
- ÔåÆ Stable (consistent performance)
- Ôåÿ´©Å Declining (quality regression detected)
...
if (( $(echo "$score < 0.95" | bc -l) )); then
  echo "ÔØî Verification failed with score: $score"
  echo "Run 'npx ruflo@alpha verify check --verbose' for details"
  exit 1
fi

echo "Ô£à Verification passed with score: $score"
```
...
- `0`: Verification passed (score ÔëÑ threshold)
