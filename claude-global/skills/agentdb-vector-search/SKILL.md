...
Implements vector-based semantic search using AgentDB's high-performance vector database with **150x-12,500x faster** operations than traditional solutions. Features HNSW indexing, quantization, and sub-millisecond search (<100┬Ás).
...
```typescript
const adapter = await createAgentDBAdapter({
  quantizationType: 'binary',  // 768-dim ÔåÆ 96 bytes
});
```
...
```typescript
const adapter = await createAgentDBAdapter({
  quantizationType: 'scalar',  // 768-dim ÔåÆ 768 bytes
});
```
...
```typescript
const adapter = await createAgentDBAdapter({
  quantizationType: 'product',  // 768-dim ÔåÆ 48-96 bytes
});
```
...
- **Sub-millisecond retrieval** (<100┬Ás)
...
- **Vector Search**: <100┬Ás (HNSW indexing)
