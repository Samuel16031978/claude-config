# Risk Assessment Report (NIST SP 800-30 Rev 1) ÔÇö Worked Example
...
- **Scope:** Internet-facing application tier ÔÇö customer web portal, its identity provider (IdP), the API gateway, and the data lake behind them.
...
- **Scales:** Five-level qualitative (Very Low ÔåÆ Very High). Risk computed via the 800-30 Rev 1 reference 5├ù5 matrix (see `references/standards.md`). This matrix was agreed during Prepare and is locked for the cycle.
...
| R-04 | Employee misconfigures a data-lake bucket to public | Accidental | ÔÇö |
| R-03 | SIEM storage disk fails, corrupting un-backed-up logs | Structural | ÔÇö |
...
## 6. Risk Register
*(generated by `scripts/process.py`; likelihood ├ù impact ÔåÆ risk on the 800-30 5├ù5 matrix, ranked highest-first)*

| ID | Threat event | Source | Asset | ATT&CK | Likelihood | Impact | Risk | Key vulnerabilities | Treatment | Residual | Owner |
|---|---|---|---|---|---|---|---|---|---|---|---|
| R-01 | Phish ÔåÆ lateral movement to IdP | Adversarial | Identity provider | T1566, T1021 | High | Very High | **Very High** | No phishing-resistant MFA; flat network | Mitigate | Moderate | IAM Lead |
...
| R-04 | Public S3 misconfiguration | Accidental | Data lake | ÔÇö | Moderate | High | **Moderate** | No SCP guardrail | Mitigate | Low | Cloud |
| R-03 | Disk failure corrupts logs | Structural | SIEM storage | ÔÇö | Low | Moderate | **Low** | No RAID; no offsite backup | Accept | Low | SOC |

## 7. Top Risks Summary (for leadership)
1. **R-01 ÔÇö Very High.** A successful phish into our identity provider is the single most damaging plausible event: it grants broad access and, on a flat network, fast lateral movement. **Recommendation:** deploy phishing-resistant MFA (FIDO2) and segment the IdP ÔÇö this is the highest-leverage control this fiscal year.
2. **R-02 ÔÇö High.** Edge-VPN ransomware is a sector-common entry path. **Recommendation:** emergency-patch the appliance, restrict the management plane, and validate offline backups.
3. **R-04 ÔÇö Moderate.** Cloud misconfiguration is likely but bounded; a preventive SCP guardrail collapses the likelihood cheaply.
