...
- When the organization needs a real risk *assessment* ÔÇö an analysis of specific threats, likelihoods, and impacts ÔÇö rather than a maturity score against a framework. (Maturity tells you how mature your practices are; a risk assessment tells you what could hurt you and how badly.)
- When another framework requires a documented risk analysis as a mandatory input: NIST CSF (ID.RA), ISO 27001 (Clause 6.1.2), NIST RMF / 800-37 (the Prepare and Select steps), SOC 2 (CC3), PCI DSS, or HIPAA (┬º164.308(a)(1)(ii)(A)).
...
- **Scope** ÔÇö organizational tier (Tier 1/2/3), systems, and time horizon.
- **Assumptions and constraints** (e.g., assume an external adversarial threat with moderate capability).
- **Information sources** ÔÇö threat, vulnerability, and impact inputs.
- **Risk model and analytic approach** ÔÇö the factors (threat source, threat event, vulnerability, likelihood, impact) and the scales (qualitative Very LowÔÇôVery High, or semi-quantitative 0ÔÇô10). Lock these now.

### 2. Conduct the assessment
Work through the analytic tasks in order. The 800-30 appendices provide the reference taxonomies (DÔÇôI).

**2a. Identify threat sources (Appendix D).** Classify by type: **Adversarial** (individuals, groups, nation-states ÔÇö characterize capability, intent, targeting), **Accidental** (user error), **Structural** (equipment/software failure), **Environmental** (natural disasters, infrastructure outages).
...
**2e. Determine impact (Appendix H).** Magnitude of harm if the event succeeds ÔÇö to operations, assets, individuals, other organizations, or the nation. Express against the agreed scale and in business terms.

**2f. Determine risk (Appendix I).** Risk is a function of likelihood and impact. Plot each threat event on the risk matrix (e.g., likelihood ├ù impact ÔåÆ Very Low ÔÇª Very High). Record the risk level, the contributing factors, and uncertainty/assumptions.
...
Hand the ranked register to risk owners. For each risk choose a treatment ÔÇö **mitigate** (add/strengthen controls), **transfer** (insurance, contractual), **avoid** (stop the activity), or **accept** (document residual risk with an authorizing signature). Re-score residual risk after planned controls to show the post-treatment position.
...
- **NIST SP 800-30 Rev 1** ÔÇö the methodology and Appendices DÔÇôI taxonomies (threat sources, events, vulnerabilities, likelihood, impact, risk).
- **NIST SP 800-39** ÔÇö enterprise risk-management context (three tiers).
- **MITRE ATT&CK** ÔÇö to enumerate and ground adversarial threat events in observed TTPs.
- **Vulnerability scanners / pen-test reports** ÔÇö empirical vulnerability input.
- **Threat intel sources** ÔÇö sector ISACs, vendor feeds, internal incident history for likelihood grounding.
- **GRC / risk-register tooling** ÔÇö spreadsheet, or platforms such as ServiceNow GRC, Archer, OneTrust, to store and track the register.
- **FAIR** (optional) ÔÇö a quantitative model if leadership wants risk expressed in dollar ranges rather than qualitative bands.
...
1. **Purpose, scope, and tier** ÔÇö what was assessed and why.
2. **Assumptions, constraints, and risk model** ÔÇö the factors and scales used (so results are reproducible).
3. **Threat sources** ÔÇö by type, with adversarial characterization.
4. **Threat events** ÔÇö each with affected assets and ATT&CK mapping where adversarial.
5. **Vulnerabilities and predisposing conditions** ÔÇö tied to threat events.
6. **Risk register** ÔÇö table: ID, threat event, asset, likelihood, impact, **risk level**, contributing vulnerabilities, recommended treatment, owner, residual risk.
7. **Top risks summary** ÔÇö ranked, in business terms, for leadership.
8. **Maintenance plan** ÔÇö refresh cadence and re-assessment triggers.

Use `scripts/process.py` to score the register from a risk-input JSON (likelihood ├ù impact ÔåÆ risk level on a configurable matrix), rank risks, and emit the register table.
