...
def render(inv, results):
    lines = [f"# Cloud Deception Deployment Record",
             f"\n_Account: {inv.get('account', 'UNKNOWN')} ÔÇö generated {date.today().isoformat()}_\n",
             "## 1. Decoy inventory & detection wiring",
             "| Decoy | Cloud | Type | Placement | Detection (sourceÔåÆruleÔåÆsink) | Playbook | Validated | Status |",
             "|---|---|---|---|---|---|---|---|"]
    for d, issues in results:
        det = d.get("detection") or {}
        wiring = f"{det.get('source','?')}ÔåÆ{det.get('rule','?')}ÔåÆ{det.get('sink','?')}"
        val = d.get("validated") or {}
    ...
    if problem:
        ...
        lines.append("\n## 2. Issues\nNone ÔÇö all decoys have detection wiring, least privilege, and validation.")

    lines.append("\n## 3. Maintenance")
    lines.append("- Rotation cadence: TBD  ┬À  Review owner: TBD")
