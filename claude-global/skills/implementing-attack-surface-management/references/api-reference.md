...
| `api.search_cursor(query)` | ÔÇö | Generator that auto-paginates all results | `query`, `minify` |
| `api.scan(ips)` | `POST /shodan/scan` | Request on-demand scan of IPs/netblocks | `ips` |
| `api.dns.resolve(hosts)` | `GET /dns/resolve` | Hostname ÔåÆ IP | `hostnames` |
| `api.dns.reverse(ips)` | `GET /dns/reverse` | IP ÔåÆ hostname | `ips` |
| `api.info()` | `GET /api-info` | Remaining query/scan credits, plan | ÔÇö |
...
- **Scan credits**: separate monthly budget consumed by `api.scan()` ÔÇö 1 scan credit per host requested.

### Error Codes
`401` invalid API key ┬À `403` access denied / plan lacks feature ┬À `429` rate-limit or out of credits ┬À `404` IP not found in index. SDK raises `shodan.APIError` with the message.
...
### Rate Limits
Tiered by plan. Free/community tier is limited (low queries/month and a modest requests-per-second cap); paid Platform tiers raise both. `429 Too Many Requests` when exceeded ÔÇö the SDK backs off and retries.

### Error Codes
`401` bad credentials ┬À `403` plan restriction ┬À `404` not found ┬À `422` malformed query ┬À `429` rate limit.
...
### subfinder ÔÇö passive subdomain enumeration
| Flag | Purpose |
|---|---|
...
### httpx ÔÇö HTTP probing / fingerprinting
| Flag | Purpose |
|---|---|
| `-sc` | Status code |
| `-cl` | Content length |
...
### nuclei ÔÇö template-based vulnerability scanning
| Flag | Purpose |
|---|---|
...
### Rate Limits
No vendor-imposed API rate limit for local execution ÔÇö you control load with `-rl` (requests/sec) and `-c` (concurrency). Respect target scope/authorization and any provider key limits (Shodan 1 req/s, Censys/SecurityTrails monthly quotas) consumed via subfinder's passive sources.
