# HIPAA Security Rule Gap Assessment ÔÇö Worked Example
...
- **ePHI data map:** Patient intake ÔåÆ EHR (SaaS, BAA in place) ÔåÆ claims to billing BA ÔåÆ encrypted backup. Reference diagram `ephi-flow-v2`.

## 2. Risk Analysis Summary (feeds Risk Management)
| Top risk to ePHI | Likelihood | Impact | Note |
|---|---|---|---|
| Phishing ÔåÆ EHR credential theft | High | High | No MFA on remote EHR access |
...
> The risk analysis is **dated and documented** ÔÇö this is the first artifact OCR requests.

## 3. Safeguard Status
*(scored by `scripts/process.py`; weighted readiness Ôëê 62% in this example)*

| Specification | Section | Requirement | Status | Alt. documented |
|---|---|---|---|---|
| 164.308(a)(1)(ii)(A) Risk Analysis | Administrative | required | **gap** | ÔÇö |
| 164.308(a)(1)(ii)(B) Risk Management | Administrative | required | partial | ÔÇö |
| 164.308(a)(2) Assigned Security Responsibility | Administrative | required | implemented | ÔÇö |
| 164.310(d)(2)(i) Disposal | Physical | required | implemented | ÔÇö |
| 164.312(a)(1) Unique User ID | Technical | required | implemented | ÔÇö |
...
**OCR-priority flag:** Risk Analysis is a gap ÔåÆ remediate first.
...
## 5. Breach-Notification Readiness (┬º┬º164.400ÔÇô414)
...
## 6. 2025 NPRM Gap View (PROPOSED ÔÇö not yet final)
...
1. **Risk Analysis (164.308(a)(1)(ii)(A)) ÔÇö required gap, OCR-priority.** Complete and date a full SRA (use HHS SRA Tool + NIST 800-30). **Owner:** Security Official. **Due:** 30 days.
2. **Risk Management (164.308(a)(1)(ii)(B)) ÔÇö required, partial.** Build the remediation plan that flows from the SRA. **Due:** 45 days.
3. **Encryption (164.312(a)(2)(iv)) ÔÇö addressable, partial.** Enforce full-disk + in-transit encryption (also clears NPRM + breach safe harbor). **Due:** 60 days.
4. Verify the billing BA's safeguards (request SOC 2). **Due:** 60 days.
5. Re-run the scorer after each fix; target ÔëÑ 95% weighted readiness with zero required gaps.
