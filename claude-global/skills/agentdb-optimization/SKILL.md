...
**Performance**: <100┬Ás vector search, <1ms pattern retrieval, 2ms batch insert for 100 vectors.
...
# Results show:
# Ô£à Pattern Search: 150x faster (100┬Ás vs 15ms)
# Ô£à Batch Insert: 500x faster (2ms vs 1s for 100 vectors)
# Ô£à Large-scale Query: 12,500x faster (8ms vs 100s at 1M vectors)
# Ô£à Memory Efficiency: 4-32x reduction with quantization
```
...
const adapter = await createAgentDBAdapter({
  ...
  // 768-dim float32 (3072 bytes) ÔåÆ 96 bytes binary
  // 1M vectors: 3GB ÔåÆ 96MB
});
```
...
const adapter = await createAgentDBAdapter({
  ...
  // 768-dim float32 (3072 bytes) ÔåÆ 768 bytes (uint8)
  // 1M vectors: 3GB ÔåÆ 768MB
});
```
...
const adapter = await createAgentDBAdapter({
  ...
  // 768-dim float32 (3072 bytes) ÔåÆ 48-96 bytes
  // 1M vectors: 3GB ÔåÆ 192MB
});
```
...
// Search with HNSW (100┬Ás vs 15ms linear scan)
...
```typescript
// ÔØî SLOW: Individual inserts
for (const doc of documents) {
  await adapter.insertPattern({ /* ... */ });  // 1s for 100 docs
}

// Ô£à FAST: Batch insert
...
// Expected: <50┬Ás search, 90-95% accuracy
```
...
// Expected: <100┬Ás search, 98-99% accuracy
```
...
// Expected: <200┬Ás search, 100% accuracy
```
...
// Expected: <100┬Ás search, ~10MB for 100K vectors
```
...
|-----------|-------------|-----------------|-----------|-------------|
| Search | 10K | 15ms | 100┬Ás | 150x |
| Search | 100K | 150ms | 120┬Ás | 1,250x |
