# CMMC Level 2 ÔÇö Standards & Reference
...
> Always confirm current status at the source ÔÇö acquisition rules and phase dates have moved before. Authoritative: https://dodcio.defense.gov/CMMC/ and the eCFR for 32 CFR Part 170.
...
## NIST SP 800-171 Rev 2 ÔÇö the 14 families (110 requirements)
| ┬º | Family | # reqs |
|---|---|---|
...
## DoD Assessment Methodology ÔÇö SPRS scoring
...
- Maximum score **110**; the methodology floor is **ÔêÆ203** (more is deducted than the 110 starting points because of the weighting).
- The complete per-requirement point assignment is published in the **DoD NIST SP 800-171 Assessment Methodology** ÔÇö use that document for the authoritative weight of each control rather than estimating.

## POA&M rules under the CMMC rule (32 CFR Part 170)
- A **Conditional** Level 2 status is allowed only if the assessment score is **at least 80% (ÔëÑ 88 of 110)**.
- Only **POA&M-eligible** requirements may be deferred. The highest-weighted security requirements generally **must be fully met** and **cannot** sit on a POA&M ÔÇö verify each item's eligibility against the rule.
- All POA&M items must be **closed within 180 days**; a closeout assessment then converts **Conditional ÔåÆ Final**.

## Scoping categories (CMMC Level 2 Scoping Guide)
| Category | Treatment |
|---|---|
| CUI Assets | Process/store/transmit CUI ÔÇö assessed against applicable requirements. |
| Security Protection Assets | Provide security to the CUI environment ÔÇö in scope. |
| Contractor Risk Managed Assets | Capable of handling CUI but not intended to ÔÇö managed by policy/config. |
| Specialized Assets | IoT/OT, GFE, test equipment ÔÇö documented, limited assessment. |
| Out-of-Scope Assets | Isolated from CUI ÔÇö not assessed. |
...
| GV.SC-01 | Supply-chain risk management ÔÇö flowdown to subs / ESPs. |
