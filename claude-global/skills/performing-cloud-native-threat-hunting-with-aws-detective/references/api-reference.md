...
Detective uses standard AWS IAM authentication ÔÇö no separate API key. Credentials resolve through the SDK credential provider chain (environment variables, `~/.aws/credentials` profile, EC2/ECS/EKS/Lambda role, or SSO).
...
### `list_indicators` ÔÇö verified parameters

`GraphArn` (string, required), `InvestigationId` (string, required), `IndicatorType` (string, optional filter), `NextToken` (string ÔÇö pagination token; **expires after 24 hours**), `MaxResults` (integer). Valid `IndicatorType` values:

`TTP_OBSERVED` ┬À `IMPOSSIBLE_TRAVEL` ┬À `FLAGGED_IP_ADDRESS` ┬À `NEW_GEOLOCATION` ┬À `NEW_ASO` (new autonomous system org) ┬À `NEW_USER_AGENT` ┬À `RELATED_FINDING` ┬À `RELATED_FINDING_GROUP`

### `get_investigation` ÔÇö verified
...
`list_investigations` ÔåÆ `InvestigationDetails[]`:
...
`list_indicators` ÔåÆ `Indicators[]`: each has `IndicatorType` plus an `IndicatorDetail` union populated for the matching type (e.g. `FlaggedIpAddressDetail`, `ImpossibleTravelDetail`, `NewGeolocationDetail`, `TTPsObservedDetail` carrying MITRE ATT&CK tactic/technique).
...
Throttling returns `TooManyRequestsException`; boto3 retries with exponential backoff. There is no per-request monetary charge for the API itself ÔÇö Detective is billed by **volume of log data ingested** into the behavior graph (GB/month, tiered).
